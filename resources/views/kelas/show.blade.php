            ...
            <div class="p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-lg font-semibold text-green-600">{{ $attendanceToday->total_masuk ?? 0 }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">Sudah Pulang</span>
                        <span class="text-lg font-semibold text-blue-600">{{ $attendanceToday->total_pulang ?? 0 }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">Terlambat</span>
                        <span class="text-lg font-semibold text-yellow-600">{{ $attendanceToday->total_terlambat ?? 0 }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                            ...
                            @foreach($kelas->siswa as $siswa)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($siswa->foto)
                                            <img class="h-10 w-10 rounded-full object-cover" src="{{ asset('storage/' . $siswa->foto) }}" alt="{{ $siswa->nama_lengkap }}">
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            <a href="{{ route('siswa.show', $siswa) }}" class="text-indigo-600 hover:text-indigo-900">Detail</a>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($kelas->jadwalPelajaran as $jadwal)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $jadwal->jam_masuk }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $jadwal->jam_pulang }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $jadwal->keterangan ?? '-' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            @else
                <div class="text-center py-8">
                    <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3a4 4 0 118 0v4M5 21h14l1-6H4l1 6z"></path>
                    </svg>
