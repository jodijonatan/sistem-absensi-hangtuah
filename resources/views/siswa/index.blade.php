...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('siswa.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-sm text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <form method="GET" action="{{ route('siswa.index') }}" class="space-y-4 md:space-y-0 md:flex md:items-center md:space-x-4">
                <!-- Search -->
                <div class="flex-1">
                    <input type="text" 
                           name="search" 
                ...
                <!-- Class Filter -->
                <div>
                    <select name="kelas_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                ...
                <!-- RFID Status Filter -->
                <div>
                    <select name="rfid_status" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                        <option value="">Semua Status</option>
                        <option value="registered" {{ request('rfid_status') == 'registered' ? 'selected' : '' }}>Terdaftar RFID</option>
                        <option value="unregistered" {{ request('rfid_status') == 'unregistered' ? 'selected' : '' }}>Belum Terdaftar</option>
                    </select>
                </div>
                    ...
                    @forelse($siswa as $student)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        @if($student->foto)
                                            <img class="h-10 w-10 rounded-full object-cover" src="{{ Storage::url($student->foto) }}" alt="">
                                        @else
                                            <div class="h-10 w-10 rounded-full bg-gray-300 flex items-center justify-center">
                                                <span class="text-sm font-medium text-gray-700">{{ substr($student->nama_lengkap, 0, 1) }}</span>
                                            </div>
                                        @endif
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                <a href="{{ route('siswa.show', $student) }}" class="text-indigo-600 hover:text-indigo-900">Lihat</a>
                                <a href="{{ route('siswa.edit', $student) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a>
                                <form method="POST" action="{{ route('siswa.destroy', $student) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Yakin ingin menghapus siswa ini?')">
                                        Hapus
                                    </button>
                                </form>
                            </td>
