        ...
        <div class="p-8 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-blue-100 text-lg">{{ ucfirst($user->role) }} - Sistem Absensi RFID</p>
            ...
            <div class="p-6">
                <div class="flex items-center">
                    <div class="flex-1">
                        ...
                        <div class="flex items-center mt-2">
                            <span class="text-sm text-emerald-600 font-medium">{{ $totalSiswa > 0 ? round(($todayPresent / $totalSiswa) * 100) : 0 }}% kehadiran</span>
                        </div>
                    </div>
                    <div class="flex-shrink-0">
            ...
            <div class="bg-white overflow-hidden shadow-lg rounded-xl border border-gray-100">
                <div class="p-6 border-b border-gray-100">
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('absensi.index') }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">Lihat Semua</a>
                    </div>
                </div>
                <div class="p-6">
                    @if($recentAttendance->count() > 0)
                        <div class="space-y-4">
                            @foreach($recentAttendance as $absensi)
                                <div class="flex items-center space-x-4 p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors">
                                    <div class="flex-shrink-0">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="font-semibold text-gray-900 truncate">{{ $absensi->siswa->nama_lengkap }}</p>
                                        <p class="text-sm text-gray-600">{{ $absensi->siswa->kelas->nama_kelas }} • {{ $absensi->waktu_tap->format('H:i') }}</p>
                                    </div>
                                    <div class="flex-shrink-0">
                            ...
                            @foreach($classSummary as $summary)
                                <div class="border border-gray-100 rounded-lg p-4 hover:border-blue-200 transition-colors">
                                    <div class="flex items-center justify-between mb-3">
                                        <h4 class="font-semibold text-gray-900">{{ $summary['kelas']->nama_kelas }}</h4>
                                        <span class="text-lg font-bold text-blue-600">{{ $summary['persentase_kehadiran'] }}%</span>
                                    </div>
                                    <div class="grid grid-cols-2 gap-2 text-sm mb-3">
                                        <div class="text-center p-2 bg-green-50 rounded">
                                            <div class="font-semibold text-green-700">{{ $summary['hadir_hari_ini'] }}</div>
                                            <div class="text-green-600 text-xs">Hadir</div>
                                        </div>
                                        <div class="text-center p-2 bg-red-50 rounded">
                                            <div class="font-semibold text-red-700">{{ $summary['tidak_hadir'] }}</div>
                                            <div class="text-red-600 text-xs">Tidak Hadir</div>
                                        </div>
                                    </div>
                                    <div class="w-full bg-gray-200 rounded-full h-2">
                                        <div class="bg-gradient-to-r from-green-500 to-green-600 h-2 rounded-full transition-all duration-500" style="width: {{ $summary['persentase_kehadiran'] }}%"></div>
                                    </div>
                    ...
                    @else
                        <div class="text-center py-8">
                            <svg class="mx-auto h-12 w-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5"></path>
                            </svg>
    ...
    <div class="bg-white overflow-hidden shadow-lg rounded-xl border border-gray-100">
        ...
        <div class="p-6">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <a href="{{ route('siswa.create') }}" class="group flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-200 rounded-xl hover:border-blue-300 hover:bg-blue-50 transition-all duration-200">
                ...
                <a href="{{ route('kelas.create') }}" class="group flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-200 rounded-xl hover:border-green-300 hover:bg-green-50 transition-all duration-200">
                    <div class="w-12 h-12 bg-green-100 group-hover:bg-green-200 rounded-xl flex items-center justify-center mb-3 transition-colors">
                ...
                <a href="{{ route('absensi.reports') }}" class="group flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-200 rounded-xl hover:border-purple-300 hover:bg-purple-50 transition-all duration-200">
                ...
                <a href="{{ route('absensi.create') }}" class="group flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-200 rounded-xl hover:border-orange-300 hover:bg-orange-50 transition-all duration-200">
                    <div class="w-12 h-12 bg-orange-100 group-hover:bg-orange-200 rounded-xl flex items-center justify-center mb-3 transition-colors">
