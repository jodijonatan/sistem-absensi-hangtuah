    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <form method="POST" action="{{ route('absensi.store') }}" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="datetime-local" 
                           name="waktu_tap" 
                           id="waktu_tap" 
                           value="{{ old('waktu_tap', now()->format('Y-m-d\TH:i')) }}" 
                           required
                           class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                    @error('waktu_tap')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="status" 
                        ...
                        <option value="pulang_awal" {{ old('status') === 'pulang_awal' ? 'selected' : '' }}>Pulang Awal</option>
                    </select>
                    @error('status')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-blue-700">
                        <ul class="list-disc list-inside space-y-1">
                            <li>Input manual digunakan untuk koreksi data atau siswa yang tidak memiliki kartu RFID</li>
            ...
            <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                <a href="{{ route('absensi.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-500 border border-transparent rounded-md font-semibold text-sm text-white uppercase tracking-widest hover:bg-gray-600 focus:bg-gray-600 active:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    Batal
                </a>
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                <div>
                    ...
                    <ul class="space-y-1">
                        <li>• Siswa lupa membawa kartu RFID</li>
                        <li>• Kartu RFID rusak atau tidak terbaca</li>
